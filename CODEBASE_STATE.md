...
**Current scaffold status:** *Infinite-loop auto-scroll runner + comprehensive demo shipped. Level-01 is a horizontally looping 3600px chunk (grass biome, sage NPC, flight orb, shadow-boss mid-loop, no level-exit → endless). Level-02 is a bounded space-biome track (240 px/s scroll, warrior NPC, shadow boss, looping back to level-01). Player drifts forward at `cruise × BASE_FORWARD_RATIO` when idle; flight hovers at `cruise × FLY_IDLE_RATIO`. `Phase.freezesWorld` replaces scattered phase-id checks. NPC dialogue auto-fires on zone overlap (auto-scroll leaves no window for a press-E prompt).*
**Scaffold last updated:** 2026-04-26;15:30

---

## 13.1 Directory map

*Last updated: 2026-04-26;15:30*
...
│   │   │                      #   DialogueDef、BossDef、事件 payload
│   │   ├── scenes/
...
│   │   │                          # PhaseController；loop 模式每帧调
│   │   │                          # LevelRunner.tickSpawner；按 phase.freezesWorld
│   │   │                          # 决定是否推进世界
│   │   ├── entities/
│   │   │   ├── player/
...
*Last updated: 2026-04-26;15:30. Source of truth: `src/contents/entities/**` and `src/contents/systems/level-runner.ts` (静态对象).*
...
|---|---|---|---|---|
| Platforms | `Phaser.Physics.Arcade.StaticGroup` + 独立 `TileSprite` 做视觉 | `tile-{biome}`（按 `LevelDef.biome`） | `PlatformSegmentDef` | Static body setDisplaySize+refreshBody；TileSprite 独立显示避免拉伸糊图；loop 模式下每个 chunk 复制一份 |
...
*Last updated: 2026-04-26;15:30. Source of truth: `src/contents/types.ts` (游戏类型) + scattered local `interface Props` in Vue SFCs.*
